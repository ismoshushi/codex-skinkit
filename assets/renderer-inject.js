((cssText, artDataUrl, themeConfig) => {
  ...
  const applyTheme = (root) => {
    ...
    root.style.setProperty("--dream-skin-name", cssString(THEME.name || "Codex SkinKit"));
  ...
  const ensure = () => {
    ...
    chrome.querySelector(".dream-skin-brand b").textContent = THEME.name || "Codex SkinKit";
    chrome.querySelector(".dream-skin-brand small").textContent = THEME.brandSubtitle || "CODEX SKINKIT";
    chrome.querySelector(".dream-skin-status span").textContent = THEME.statusText || "SKINKIT ONLINE";
  ...
  window[STATE_KEY] = { ensure, cleanup, observer, timer, scheduler, resizeHandler, artUrl, version: VERSION, themeId: THEME.id || "custom" };
  ensure();
