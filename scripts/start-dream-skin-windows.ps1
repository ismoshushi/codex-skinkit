...
$ready = Test-CodexRendererTarget $Port
...
if (-not $ready) {
  ...
  while (-not (Test-CodexRendererTarget $Port) -and (Get-Date) -lt $deadline) { Start-Sleep -Milliseconds 400 }
  if (-not (Test-CodexRendererTarget $Port)) { Fail "Codex did not expose a verified renderer on loopback port $Port." }
}
...
$injectorInfo = Get-CimInstance Win32_Process -Filter "ProcessId=$($injectorProcess.Id)"
$newState = [ordered]@{ schemaVersion=5; platform="win32-$env:PROCESSOR_ARCHITECTURE"; skinVersion=$SkinVersion; port=$Port; injectorPid=$injectorProcess.Id; injectorCreatedAt=$injectorInfo.CreationDate.ToString('o'); injectorPath=$Injector; nodePath=$Node; nodeVersion=$NodeVersion; codexExe=$CodexExe; codexVersion=$CodexVersion; codexPid=$main.ProcessId; codexCreatedAt=$main.CreationDate.ToString('o'); projectRoot=$ProjectRoot; themeDir=$ThemeDir; createdAt=(Get-Date -Format o) }
